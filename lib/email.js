...
export async function sendAbandonedSessionAlert(draft) {
  ...
  await resend.emails.send({
    from: FROM,
    to: [SALES_EMAIL],
    subject: `[Abandoned Onboarding] ${displayName} – Follow Up Required`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
        <h2 style="color:#111827">Abandoned Onboarding Alert</h2>
        <p style="color:#6b7280">A customer started onboarding but did not complete it. They may need follow-up.</p>
        <table style="width:100%;border-collapse:collapse;font-size:14px">
          ...
          <tr><td style="padding:6px 0;color:#9ca3af">Last Active Step</td><td style="color:#111827">Step ${draft.step}</td></tr>
